...
Cette annexe reprend les quatre procédures les plus courantes, une par espace métier, telles qu'elles seraient remises aux utilisateurs lors de la formation.
...
\section*{Annexe B : extraits de code}

Les trois extraits qui suivent portent sur les points où une erreur d'implémentation aurait des conséquences directes sur la validité de la procédure ou sur la confidentialité des dossiers.

\subsection*{B.1 : dépôt d'une plainte et rattachement au commissariat}
...
\subsection*{B.2 : rédaction assistée du procès-verbal}
...
\subsection*{B.3 : cloisonnement des accès au niveau du SGBD}
...
\section*{Annexe C : fiches de tests}

La recette a été conduite manuellement sur un jeu de dossiers couvrant les huit natures d'infraction gérées. Le tableau~\ref{tab:fiches_tests} en consigne les dix cas retenus : quatre portent sur le parcours de dépôt, deux sur le cloisonnement des accès, et les autres sur la cotation, les documents produits et la dictée vocale.

La colonne \textbf{Verdict} demande une lecture attentive. « Conforme » signifie que le comportement observé correspond à l'attente. « Refusé comme attendu » qualifie les deux tests de sécurité, où c'est le \textbf{rejet} qui constitue le succès. Enfin, deux lignes ne sont ni des réussites ni des échecs mais des \textbf{points signalés} : T-08 montre qu'un procès-verbal peut être validé sans relecture (le système le trace au nom de l'enquêteur mais ne l'empêche pas), et T-10 constate que la transcription vocale exige des corrections manuelles. Ces deux constats alimentent les préconisations du chapitre~\ref{ch:bilan}.
...
Le tableau~\ref{tab:glossaire} rassemble les termes employés dans ce mémoire. Il en mêle délibérément deux familles, car le sujet se tient à leur intersection : d'un côté le vocabulaire de la \textbf{procédure} (cotation, procès-verbal, récépissé), dont la signification est fixée par l'usage des commissariats et non par l'informatique ; de l'autre le vocabulaire \textbf{technique} : API REST, jeton JWT, politique d'accès par ligne.

Les définitions du premier groupe reprennent le sens que leur donnent les personnels rencontrés lors de l'audit. Celles du second précisent l'emploi qui en est fait dans la plateforme, qui n'est pas toujours l'emploi le plus courant : ainsi l'entrée « IA générative » rappelle que le modèle ne produit ici que des propositions soumises à validation.
